...
import com.liany.clientmodel.debug.ClientApplication;
import com.liany.clientmodel.diagnose.IsCheckEntity;
...
//                        dict.getDictValue().equals("其他侵入手段") ) {
    ...
    public static boolean selectOtherItem(List<sysDict> dicts, sysDict dict) {
        boolean isOther = false;
        for(com.liany.clientmodel.diagnose.sysDict sysDict : dicts) {
            if(sysDict.getDictLevel().equals(dict.getDictLevel()) && sysDict.getDictKey().equals(dict.getDictKey())) {
                List<com.liany.clientmodel.diagnose.sysDict> selectValue = new ArrayList<>();
                for(com.liany.clientmodel.diagnose.sysDict dict1 : dicts) {
