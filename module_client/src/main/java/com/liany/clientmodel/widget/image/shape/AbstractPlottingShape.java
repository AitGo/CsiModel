package com.liany.clientmodel.widget.image.shape;
...
public abstract class AbstractPlottingShape extends DrawableShape implements IEventful, IPlottingSaveable {
    ...
    @Override
    public void dragDraw(float x, float y, int type_shape) {
        setEnd(x, y, type_shape);
        configBoundingBox();
    }
    ...
    public void destructuring(PlottingRaw raw, Matrix referenceMatrix) {
        ...
        caculate(values[0], values[1], values[2], values[3], 0);
    }
     ...
     * 并对局部坐标系进行设置
     */
    private void caculate(float x1, float y1, float x2, float y2, int type) {
        //1、计算形状长度
        float a = y2 - y1;     //对边
        float b = x2 - x1;     //邻边
    ...
    private void setBegin(float x, float y) {
        ...
        caculate(x, y, getReferencePosition().x, getReferencePosition().y,0);
     ...
     * @param y Global坐标值
     */
    private void setEnd(float x, float y, int type_shape) {

        caculate(getReferencePosition().x, getReferencePosition().y, x, y,type_shape);
