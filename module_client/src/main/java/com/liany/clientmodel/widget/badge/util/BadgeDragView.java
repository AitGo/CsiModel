...
package com.liany.clientmodel.widget.badge.util;
...
class BadgeDragView extends View {
    private BadgeFactory mBadgeViewHelper;
    ...
    public BadgeDragView(Context context, BadgeFactory badgeViewHelper) {
        super(context);
    ...
    private void initStick() {
        mMaxDragRadius = ViewUtils.dp2px(getContext(), 10);
        mDragStickRadiusDifference = ViewUtils.dp2px(getContext(), 1);
    }
    ...
    private void drawStick(Canvas canvas) {
        ...
        // 通过几何图形工具获取交点坐标
        mDragPoints = ViewUtils.getIntersectionPoints(mDragCenter, mDragRadius, lineK);
        mStickPoints = ViewUtils.getIntersectionPoints(mStickCenter, currentStickRadius, lineK);

        // 3. 获取控制点坐标
        mControlPoint = ViewUtils.getMiddlePoint(mDragCenter, mStickCenter);

        // 保存画布状态
        canvas.save();
        canvas.translate(0, -ViewUtils.getStatusBarHeight(mBadgeViewHelper.getRootView()));
    ...
    private float getCurrentStickRadius() {
         ...
         */
        float distance = ViewUtils.getDistanceBetween2Points(mDragCenter, mStickCenter);
        distance = Math.min(distance, mDismissThreshold);
        float percent = distance / mDismissThreshold;
        return ViewUtils.evaluate(percent, mStickRadius, mStickRadius * 0.2f);
    }
    ...
    private void handleActionMove(MotionEvent event) {
        if (mExplosionAnimator == null && getParent() != null) {
            ...
            // 处理断开事件
            if (ViewUtils.getDistanceBetween2Points(mDragCenter, mStickCenter) > mDismissThreshold) {
                mDismissAble = true;
                postInvalidate();
    ...
    private void startSpringAnim() {
        ...
        springAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator mAnim) {
                ...
                PointF p = ViewUtils.getPointByPercent(startReleaseDragCenter, mStickCenter, percent);
...
//        springAnim.setRepeatMode(ValueAnimator.INFINITE);
    ...
    private void startDismissAnim(int newX, int newY) {
        ...
        Bitmap badgeBitmap = ViewUtils.createBitmapSafely(this, rect, 1);
        if (badgeBitmap == null) {
            removeSelf();
            mBadgeViewHelper.endDragWithDismiss();
            return;
    ...
    private int getNewStartY(float rawY) {
        ...
        int newStartY = (int) rawY - badgeHeight / 2 - ViewUtils.getStatusBarHeight(mBadgeViewHelper.getRootView());
