...
public class BadgeFactory {
    ...
    private BadgeDragable mBadgeable;
     ...
     */
    private DismissDelegate mDelegage;
    private boolean mIsShowDrawable = false;

    public BadgeFactory(BadgeDragable badgeable, Context context, AttributeSet attrs, BadgeGravity defaultBadgeGravity) {
        mBadgeable = badgeable;
        initDefaultAttrs(context, defaultBadgeGravity);
        initCustomAttrs(context, attrs);
        afterInitDefaultAndCustomAttrs();
        mDropBadgeView = new BadgeDragView(context, this);
    }
    ...
    private void initDefaultAttrs(Context context, BadgeGravity defaultBadgeGravity) {
        ...
        mBadgeTextSize = ViewUtils.sp2px(context, 10);
        ...
        mBadgePadding = ViewUtils.dp2px(context, 3);
        mBadgeVerticalMargin = ViewUtils.dp2px(context, 4);
        mBadgeHorizontalMargin = ViewUtils.dp2px(context, 4);
        ...
        mDragExtra = ViewUtils.dp2px(context, 4);
    ...
    public void setBadgeTextSizeSp(int badgetextSize) {
        if (badgetextSize >= 0) {
            mBadgeTextSize = ViewUtils.sp2px(mBadgeable.getContext(), badgetextSize);
            mBadgePaint.setTextSize(mBadgeTextSize);
            mBadgeable.postInvalidate();
        }
    }
    ...
    public void setBadgeVerticalMarginDp(int badgeVerticalMargin) {
        if (badgeVerticalMargin >= 0) {
            mBadgeVerticalMargin = ViewUtils.dp2px(mBadgeable.getContext(), badgeVerticalMargin);
            mBadgeable.postInvalidate();
        }
    }
    ...
    public void setBadgeHorizontalMarginDp(int badgeHorizontalMargin) {
        if (badgeHorizontalMargin >= 0) {
            mBadgeHorizontalMargin = ViewUtils.dp2px(mBadgeable.getContext(), badgeHorizontalMargin);
            mBadgeable.postInvalidate();
        }
    }
    ...
    public void setBadgePaddingDp(int badgePadding) {
        if (badgePadding >= 0) {
            mBadgePadding = ViewUtils.dp2px(mBadgeable.getContext(), badgePadding);
            mBadgeable.postInvalidate();
        }
    }
    ...
    public void setBadgeBorderWidthDp(int badgeBorderWidthDp) {
        if (badgeBorderWidthDp >= 0) {
            mBadgeBorderWidth = ViewUtils.dp2px(mBadgeable.getContext(), badgeBorderWidthDp);
            mBadgeable.postInvalidate();
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mDragable && mIsShowBadge && mBadgeDragExtraRectF.contains(event.getX(), event.getY())) {
                    mIsDraging = true;
    ...
    public void setDragDismissDelegage(DismissDelegate delegage) {
        mDelegage = delegage;
    }
